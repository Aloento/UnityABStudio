namespace SoarCraft.QYun.UnityABStudio.Converters.ShaderConverters.SpirV {
    ...
    public class Disassembler {
        public string Disassemble(Module module) {
            return this.Disassemble(module, DisassemblyOptions.Default);
        }

        public string Disassemble(Module module, DisassemblyOptions options) {
            this.m_sb.AppendLine("; SPIR-V");
            this.m_sb.Append("; Version: ").Append(module.Header.Version).AppendLine();
            if (module.Header.GeneratorName == null) {
                this.m_sb.Append("; Generator: unknown; ").Append(module.Header.GeneratorVersion).AppendLine();
            } else {
                this.m_sb.Append("; Generator: ").Append(module.Header.GeneratorVendor).Append(' ').
                    Append(module.Header.GeneratorName).Append("; ").Append(module.Header.GeneratorVersion).AppendLine();
            }
            this.m_sb.Append("; Bound: ").Append(module.Header.Bound).AppendLine();
            this.m_sb.Append("; Schema: ").Append(module.Header.Reserved).AppendLine();
            ...
            this.m_sb.Append(lines[0]);
            for (int i = 1; i < lines.Length; i++) {
                string line = lines[i];
                int index = line.IndexOf('=');
                if (index == -1) {
                    this.m_sb.Append(' ', longestPrefix + 4);
                    this.m_sb.Append(line);
                } else {
                    int pad = Math.Max(0, longestPrefix - index);
                    this.m_sb.Append(' ', pad);
                    this.m_sb.Append(line, 0, index);
                    this.m_sb.Append('=');
                    this.m_sb.Append(line, index + 1, line.Length - index - 1);
                }
                this.m_sb.AppendLine();
            }

            string result = this.m_sb.ToString();
            this.m_sb.Clear();
            return result;
        }
