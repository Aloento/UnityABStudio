    ...
    public static class TypeTreeHelper {
        public static string ReadTypeString(TypeTree m_Type, ObjectReader reader) {
            ...
            if (read != reader.byteSize) {
                Console.WriteLine($"Error while read type, read {read} bytes but expected {reader.byteSize} bytes");
            }
            return sb.ToString();
        }

        private static void ReadStringValue(StringBuilder sb, IReadOnlyList<TypeTreeNode> m_Nodes, UnityReader reader, ref int i) {
        ...
        private static object ReadValue(IReadOnlyList<TypeTreeNode> m_Nodes, UnityReader reader, ref int i) {
            var m_Node = m_Nodes[i];
            var varTypeStr = m_Node.m_Type;
            object value;
            var align = (m_Node.m_MetaFlag & 0x4000) != 0;
        ...
        private static List<TypeTreeNode> GetNodes(IReadOnlyList<TypeTreeNode> m_Nodes, int index) {
            var nodes = new List<TypeTreeNode> { m_Nodes[index] };
