...
namespace SoarCraft::QYun::TextureDecoder {
    ...
    void TextureDecoderService::DecodeETC2A1Block(Byte* data, UInt32* outbuf) {
        ...
        if (r + dr < 0 || r + dr > 255) {
            ...
            uint_fast32_t color_set[4] = { ApplicateColorRaw(c[0]), ApplicateColor(c[1], d), ApplicateColorRaw(c[1]),
            ...
            for (int i = 0; i < 16; i++, j >>= 1, k >>= 1) {
                ...
                outbuf[WriteOrderTable[i]] = ApplicateColorAlpha(c[s], k & 1 ? -m : m, !obaq && (k & 1) && !(j & 1));
            }
        }
    }
}
